...
Application code lives in the installable package `python_template` under `src/python_template/`. After `make install` (`uv sync`), imports resolve like a normal package (`import python_template`).
...
`make install` runs `uv sync --frozen`, which installs the project in editable/dev mode along with its dependencies. Tests do not require a `.env` file.

## Makefile targets

| Target | Description |
| --- | --- |
| `make install` | Install the package and dependencies (`uv sync --frozen`) |
...
| `make dev` | Run the app (`python -m python_template.main`) |
...
4. Update the `make dev` module path (`python -m <your_package>.main`) and any `import` / `from` lines that still say `python_template`.
